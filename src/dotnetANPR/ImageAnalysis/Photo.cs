...
public class Photo : IDisposable
{
    ...
    public void Rotate(double angleDegrees)
    {
        ...
        var angleRad = Math.PI * angleDegrees / 180.0;
        var sine = (float)Math.Abs(Math.Sin(angleRad));
        var cosine = (float)Math.Abs(Math.Cos(angleRad));
        var newWidth = (int)Math.Round(Width * cosine + Height * sine);
        var newHeight = (int)Math.Round(Width * sine + Height * cosine);
    ...
    public void AdaptiveThresholding(int radius)
    {
        ...
        var sourceMatrix = GetBrightnessMatrix(); // Optimized
        ...
        for (var y = 0; y < Height; y++)
        {
            for (var x = 0; x < Width; x++)
            {
                ...
                var neighborhoodAvg = neighborhoodSum / count;
                var pixelVal = sourceMatrix[x, y] < neighborhoodAvg ? 0f : 1f;
    ...
    public HoughTransformation GetHoughTransformation()
    {
        ...
        var brightnessMatrix = GetBrightnessMatrix(); // Optimized
        for (var y = 0; y < Height; y++)
        {
            for (var x = 0; x < Width; x++)
            {
