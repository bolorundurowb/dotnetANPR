...
public class HoughTransformation
{
    ...
    public HoughTransformation(int width, int height)
    {
        ...
        for (var alpha = 0; alpha < _alphaMax; alpha++)
        {
            var rad = (alpha - 90) * Math.PI / 180.0;
    ...
    public void AddLine(int x, int y, float brightness)
    {
        ...
        for (var alpha = 0; alpha < _alphaMax; alpha++)
        {
            var radius = (int)(x * _cosCache[alpha] + y * _sinCache[alpha]);
            var radiusOffset = radius + _radiusMax; // Offset to handle negative radius
    ...
    public HoughLine GetBestLine()
    {
        ...
        for (var r = 0; r < _radiusMax * 2; r++)
        {
            for (var a = 0; a < _alphaMax; a++)
            {
